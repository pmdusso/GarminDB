...
- **2026-06-24 Training Readiness entregue (Fase 2, item parcial):** pipeline completo de prontidão de treino. Tabela `training_readiness` (SQLAlchemy, Tasks 1–2), download via endpoint `/metrics-service/metrics/trainingreadiness/` (Task 3), flag `--training_readiness` no CLI (Task 4), `ReadinessAnalyzer` com médias mensais + 7 dias recentes (Task 5), wiring no `LongitudinalReportBuilder` (Task 6) e subsection `_training_readiness` no renderer (Task 7). **Live run confirmado:** 20 dias importados (2026-06-04 a 2026-06-23), score máx 84, subsection "Prontidão de treino" aparece em Seção 4 do `--anamnesis`. **Unidade de `recovery_time`:** a API retorna **minutos** (e.g. 2287 min = ~38 h de recuperação necessária após esforço intenso); o label do renderer já diz "min", sem conversão necessária. Itens restantes da Fase 2: Training Status (sem suporte upstream — requer engenharia reversa de endpoint), decoupling cardíaco, import de potência no DB, fix da tabela `hrv`. Entregas anteriores já registradas acima: Fase 0 (clínicos), Fase 1 (potência via summary files), architecture cleanup (decoupling da stack de relatórios, deduplicação de lógica).
